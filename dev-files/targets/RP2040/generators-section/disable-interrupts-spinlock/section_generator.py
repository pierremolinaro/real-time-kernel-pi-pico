...
def generateDisableInterruptAndSpinLockHeader () :
  ...
  sCode += "spinlock.acquired: // C type: bool [2]\n"
  sCode += "  .space  2\n\n"
  return ("", sCode)
...
def generateDisableInterruptAndSpinLockSection (sectionName):
  ...
  sCode += "//--- R6 <- Saved interrupt enabled state\n"
  ...
  sCode += "//--- R7 <- Address for spinlock.acquired 16-bit variable (bool [2])\n"
  ...
  sCode += "//  - R6 contains the saved PRIMASK\n"
  sCode += "//  - R7 contains the spinlock.acquired 8-bit variable for current cpu\n"
  sCode += "  movs  r5, #1\n"
  sCode += "  strb  r5, [r7]\n"
