...
ACANSettings::ACANSettings (INIT_MODE_
...
uint32_t ACANSettings::actualBitRate (void) const {
  ...
  const uint32_t TQCount = SyncSeg + mPropagationSegment + mPhaseSegment1 + mPhaseSegment2 ;
...
uint32_t ACANSettings::samplePointFromBitStart (void) const {
  const uint32_t SyncSeg = 1 ;
  const uint32_t TQCount = SyncSeg + mPropagationSegment + mPhaseSegment1 + mPhaseSegment2 ;
  const uint32_t samplePoint = SyncSeg + mPropagationSegment + mPhaseSegment1 - mTripleSampling ;
