...
static void canReceive (USER_MODE) {
  while (1) {
    CANMessage message ;
    if (ACAN::can0.receive (MODE_ message)) {
      receivedCount0 += 1 ;
    }else if (ACAN::can1.receive (MODE_ message)) {
      receivedCount1 += 1 ;
    }else{
      waitDuring (MODE_ 1) ;
    }
...
static void initApplication (INIT_MODE) {
  ...
  kernel_createTask (MODE_ gStack2, sizeof (gStack2), canReceive) ;
  kernel_createTask (MODE_ gStack3, sizeof (gStack3), display) ;
}
